...
mainApi.post('/ai/process', async (ctx) => {
  try {
    ...
    await aiDataStorage.file(`requests/${requestId}.json`).write(
      JSON.stringify({
        prompt,
        model,
        timestamp: new Date().toISOString(),
...
mainApi.get('/metrics', async (ctx) => {
  try {
    ...
    ctx.res.body = Object.entries(metrics)
      .map(
        ([key, value]) =>
          `nitric_${key} ${typeof value === 'object' ? JSON.stringify(value) : value}`
      )
      .join('\n');
  } catch (error) {
    ctx.res.status = 500;
...
schedule('optimize-performance').cron('0 */6 * * *', async () => {
  ...
  try {
    ...
    await aiDataStorage.file('logs/optimization.json').write(
      JSON.stringify({
        timestamp: new Date().toISOString(),
        action: 'cleanup',
