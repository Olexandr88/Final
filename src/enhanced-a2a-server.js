...
app.post('/workflow', async (req, res) => {
  ...
  const invalidSteps = steps.filter((step) => !validProviders.includes(step.provider));
  ...
  if (invalidSteps.length > 0) {
    ...
    if (workflow.retry_count < workflow.retry_policy.max_retries) {
      ...
      workflow.next_retry_at = new Date(
        Date.now() + Math.pow(2, workflow.retry_count) * 1000
      ).toISOString();
    }
  }
