...
export class OptimizationOrchestrator extends EventEmitter {
  ...
  async collectSystemStatus() {
    ...
    if (this.state.realTimeEngine) {
      status.systems.realTime = this._cachedMetrics.realTime || this.state.realTimeEngine.getMetrics();
    }
    ...
    if (this.state.performanceOptimizer) {
      status.systems.performance = this._cachedMetrics.performance || this.state.performanceOptimizer.getMetrics();
    }

    if (this.state.performanceMonitor) {
      status.systems.monitor = this._cachedMetrics.monitor || this.state.performanceMonitor.getStats();
    }
