...
export class ClineIntegrationManager extends EventEmitter {
  ...
  updateMetrics() {
    ...
    this.metrics.activeIntegrations = Array.from(this.integrationPoints.values()).filter(
      (point) => point.connected
    ).length;
    this.metrics.activeWorkflows = Array.from(this.workflows.values()).filter(
      (workflow) => workflow.status === 'ready' || workflow.status === 'running'
    ).length;
    this.metrics.activeOptimizers = Array.from(this.optimizers.values()).filter(
