...
export class PerformanceMonitor extends EventEmitter {
  ...
  setupPerformanceObserver() {
    ...
    try {
      ...
      console.warn(
        '[PerformanceMonitor] Some performance entry types not supported:',
        error.message
      );
  ...
  getStats() {
    ...
    if (recentSamples.length > 0) {
      avgMemoryUsage =
        recentSamples.reduce((acc, s) => acc + s.memory?.usage || 0, 0) / recentSamples.length;
    }

    const performanceSamples = this.metrics.samples.filter((s) => s.duration !== undefined);
    if (performanceSamples.length > 0) {
      avgResponseTime =
        performanceSamples.reduce((acc, s) => acc + s.duration, 0) / performanceSamples.length;
    }
