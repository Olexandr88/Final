  ...
  startCleanup() {
    setInterval(() => {
      ...
      for (const [clientId, connection] of this.connections) {
        ...
        if (
          timeSinceLastMessage > this.connectionTimeout ||
  ...
  startMemoryPressureMonitoring() {
    setInterval(() => {
      ...
      if (memoryPressure > 0.8) {
        ...
        console.log(
          `🧠 High memory pressure detected. Reduced cache size: ${currentMax} → ${newMax}`
        );
...
uptime_seconds ${Math.floor(metrics.uptime / 1000)}
  ...
  setupWebSocket() {
    ...
    this.wsPool.on('broadcastComplete', (data) => {
      console.log(
        `📡 Broadcast complete: ${data.successCount}/${data.successCount + data.errorCount} in ${data.duration}`
      );
    });
  }
