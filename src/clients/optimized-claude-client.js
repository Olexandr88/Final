...
export class OptimizedClaudeClient extends EventEmitter {
  ...
  async _processBatch(batch) {
    ...
    const results = await Promise.allSettled(batch.map((item) => this._processMessage(item)));
  ...
  getMetrics() {
    ...
    return {
      ...
      cacheHitRate:
        this.metrics.cacheHits / Math.max(1, this.metrics.cacheHits + this.metrics.cacheMisses),
