...
export class LLMFrameworkMCPServer extends EventEmitter {
  ...
  private updateMetrics(toolName: string, responseTime: number, success: boolean): void {
    ...
    // Track tool usage
    const toolStats = this.performanceMetrics.toolUsage.get(toolName) || {
      calls: 0,
      errors: 0,
      avgTime: 0,
    };
    ...
    toolStats.avgTime =
      (toolStats.avgTime * (toolStats.calls - 1) + responseTime) / toolStats.calls;
  ...
  private emitMetrics(): void {
    const avgResponseTime =
      this.performanceMetrics.responseTime.length > 0
        ? this.performanceMetrics.responseTime.reduce((a: number, b: number) => a + b) /
    ...
    const metrics = {
      ...
      errorRate:
        this.performanceMetrics.requests > 0
          ? this.performanceMetrics.errors / this.performanceMetrics.requests
          : 0,
      avgResponseTime,
      cacheHitRate:
        this.performanceMetrics.cacheHits /
    ...
    console.log(
      `📊 Metrics: ${this.performanceMetrics.requests} requests, ${avgResponseTime.toFixed(2)}ms avg, ${(metrics.errorRate * 100).toFixed(1)}% errors`
    );
  }
  ...
  public async start(): Promise<void> {
    ...
    console.log(
      `🔧 Available tools: ${Object.keys(this.config.tools)
        .filter((t) => this.config.tools[t].enabled)
        .join(', ')}`
    );
