...
class UltraOptimizedTextAnalyzer {
  ...
  getBasicMetrics(text) {
    ...
    const avgSentencesPerParagraph =
      paragraphCount > 0 ? Math.round((sentenceCount / paragraphCount) * 10) / 10 : 0;
  ...
  getAdvancedMetrics(text) {
    ...
    const vocabularyDiversity =
      words.length > 0 ? Math.round((uniqueWords / words.length) * 10000) / 100 : 0;

    // Text complexity indicators
    const longWords = words.filter((word) => word.length > 6).length;
    const longWordPercentage =
      words.length > 0 ? Math.round((longWords / words.length) * 10000) / 100 : 0;
  ...
  detectContentType(text) {
    ...
    const primaryType =
      maxScore > 0 ? Object.keys(scores).find((key) => scores[key] === maxScore) : 'general';
    ...
    const characteristics = {
      ...
      hasDates:
        /\d{1,2}\/\d{1,2}\/\d{2,4}|\d{4}-\d{2}-\d{2}|\b(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\b/.test(
          text
        ),
    };
  ...
  getAIAnalysis(text, options) {
    const analysis = {
      ...
      summary: options.includeSummary
        ? this.aiModels.summarizer.summarize(text, { sentences: 3 })
        : null,
...
class ContentDetector {
  detect(text) {
    const detections = {
      hasPersonalInfo:
        /\b\d{3}-\d{2}-\d{4}\b|\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b/.test(text),
