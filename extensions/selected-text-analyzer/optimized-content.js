    ...
    return {
      ...
      summary: {
        ...
        criticalIssues: issues.filter(
          (i) => i.type === 'broken_images' || i.type === 'invalid_forms'
        ).length,
    ...
    button.onclick = async () => {
      try {
        ...
        const response = await new Promise((resolve) => {
          chrome.runtime.sendMessage({ action: 'analyzeGitHubPage', data: {} }, resolve);
        });
    ...
    const observer = new MutationObserver((mutations) => {
      ...
      mutations.forEach((mutation) => {
        if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
          ...
          const hasServiceAccountElements = Array.from(mutation.addedNodes).some(
            (node) =>
              node.nodeType === 1 &&
              (node.textContent?.includes('@') || node.textContent?.includes('service account'))
          );
    ...
    try {
      ...
      setTimeout(async () => {
        try {
          ...
          console.log(
            '⚠️ GCP pre-cache failed (normal if no service account data):',
            error.message
          );
        }
      }, 1000);
