...
self.addEventListener('install', (event) => {
  ...
  event.waitUntil(
    Promise.all([self.skipWaiting(), initializeCache(), setupPerformanceMonitoring()])
  );
});
...
self.addEventListener('activate', (event) => {
  ...
  event.waitUntil(Promise.all([self.clients.claim(), cleanupOldCaches(), startHeartbeat()]));
});
...
async function cleanupOldCaches() {
  ...
  if ('caches' in self) {
    ...
    const oldCaches = cacheNames.filter(
      (name) => name.startsWith('text-analyzer-') && !name.includes('v2.0.0')
    );
