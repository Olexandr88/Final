...
export const performanceOptimizer: Tool = {
  ...
  description:
    'Analyze and optimize system performance, detect bottlenecks, and apply optimizations',
  parameters: {
    type: 'object',
    properties: {
      operation: {
...
async function optimizePerformance(
  metrics: PerformanceMetrics,
  autoFix: boolean
): Promise<{ optimizations: string[]; recommendations: string[] }> {
...
async function monitorPerformance(threshold: number): Promise<void> {
  ...
  const interval = setInterval(async () => {
    ...
    if (metrics.memory.percentage > threshold || metrics.cpu > threshold) {
      console.warn(
        `Performance Alert: CPU: ${metrics.cpu.toFixed(2)}%, Memory: ${metrics.memory.percentage.toFixed(2)}%`
      );
    }
