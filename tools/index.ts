...
const toolRegistry: ToolRegistry = {
  ...
  coverallsApi: () =>
    import('./coveralls-api.js').then((m) => ({
      name: 'coveralls_api',
      ...
      async execute(params: any) {
        ...
        try {
          switch (operation) {
            case 'getRepo':
              if (!service || !username || !repo)
                throw new Error('Missing required parameters: service, username, repo');
            ...
            case 'getSourceFile':
              if (!jobId || !filePath)
                throw new Error('Missing required parameters: jobId, filePath');
...
const testRepoTool: ToolDefinition = {
  name: 'testRepo',
  description:
    'Test a GitHub repository with optional PR number and comprehensive test suite execution',
  version: '2.0.0',
  category: 'testing',
...
export async function loadTool(toolName: string): Promise<Tool | ToolDefinition> {
  ...
  if (!toolRegistry[toolName]) {
    throw new Error(
      `Tool '${toolName}' not found in registry. Available tools: ${Object.keys(toolRegistry).join(', ')}`
    );
  }
  ...
  try {
    ...
    if (!tool || !tool.name || !tool.execute || typeof tool.execute !== 'function') {
      throw new Error(
        `Invalid tool structure for '${toolName}'. Tool must have name and execute function.`
      );
    }
...
export async function loadTools(
  toolNames: string[]
): Promise<Record<string, Tool | ToolDefinition>> {
...
export async function loadWorkflowTools(
  workflow: keyof typeof workflowTools
): Promise<Record<string, Tool | ToolDefinition>> {
