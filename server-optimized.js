  ...
  perfMonitor.measureOperation(`http-${req.method}-${req.path}`, () => {
    res.on('finish', () => {
      ...
      if (process.env.NODE_ENV !== 'production' || responseTime > 2000) {
        console.log(
          `[${new Date().toISOString()}] ${req.method} ${req.path} - ${res.statusCode} - ${responseTime}ms [${requestId}]`
        );
      }
    });
  });
...
setInterval(() => {
  ...
  if (orchestratorMetrics.optimization) {
    metrics.optimizations = {
      ...
      failed:
        (orchestratorMetrics.optimization.total || 0) -
        (orchestratorMetrics.optimization.successful || 0),
  ...
  const healthCheck = {
    ...
    services: {
      ...
      optimization: {
        ...
        performanceOptimizer: optimizationStatus.systems?.performanceOptimizer
          ? 'active'
          : 'inactive',
      },
    },
  ...
  res.json({
    ...
    features: {
      browserHistory: {
        ...
        note: isRealHistory
          ? 'Real browser data access with intelligent caching'
          : 'Run npm run build for real browser history',
      },
      optimization: {
        realTimeEngine: optimizationStatus.systems?.realTimeEngine ? 'ACTIVE' : 'INACTIVE',
        memoryManager: optimizationStatus.systems?.memoryManager ? 'ACTIVE' : 'INACTIVE',
    ...
    endpoints: [
      {
        path: '/health',
        method: 'GET',
        description: 'Enhanced health check with optimization status',
      },
      { path: '/metrics', method: 'GET', description: 'Comprehensive Prometheus metrics' },
      {
        path: '/optimization',
        method: 'GET',
        description: 'Real-time optimization status and controls',
      },
      {
        path: '/optimization/optimize',
        method: 'POST',
        description: 'Trigger manual optimization',
      },
      {
        path: '/optimization/report',
        method: 'GET',
        description: 'Comprehensive optimization report',
      },
      {
        path: '/api/status',
        method: 'GET',
        description: 'Detailed system status with optimization metrics',
      },
      {
        path: '/history',
        method: 'GET',
        description: 'Get recent browser history with intelligent caching',
      },
      {
        path: '/history/:count',
        method: 'GET',
    ...
    performance: {
      ...
      system: {
        ...
        dataTransfer: {
          ...
          average:
            metrics.requests > 0 ? Math.round(metrics.totalDataTransferred / metrics.requests) : 0,
        },
      },
    },
...
app.get('/history', async (req, res) => {
  try {
    ...
    res.json({
      ...
      note: isRealHistory
        ? 'Real browser history with intelligent caching'
        : 'Mock data - run npm run build for real browser history',
    });
  } catch (error) {
    metrics.errors++;
    res.status(500).json({
...
app.get('/history/:count', async (req, res) => {
  try {
    ...
    res.json({
      ...
      note: isRealHistory
        ? 'Real browser history with performance optimization'
        : 'Mock data - run npm run build for real browser history',
    });
  } catch (error) {
    metrics.errors++;
...
app.get('/search', async (req, res) => {
  try {
    ...
    res.json({
      ...
      note: isRealHistory
        ? 'Real browser history search with performance optimization'
        : 'Mock data search - run npm run build for real browser history',
    });
  } catch (error) {
    metrics.errors++;
    res.status(500).json({
...
async function startServer() {
  try {
    ...
    const server = app.listen(PORT, '0.0.0.0', () => {
      ...
      console.log(
        '📊 Browser History:',
        isRealHistory ? 'Real SQLite Access with Caching' : 'Mock Implementation'
      );
