...
function cleanup() {
  for (const id of Array.from(active.intervals))
    try {
      _clearInterval(id);
    } catch {}
  active.intervals.clear();
  for (const id of Array.from(active.timeouts))
    try {
      _clearTimeout(id);
    } catch {}
  active.timeouts.clear();
  for (const it of active.listeners.splice(0))
    try {
      it.target.removeEventListener?.(it.event, it.handler, it.opts);
    } catch {}
}
...
async function withRetries(
  fn,
  { retries = 3, baseDelayMs = 250, factor = 2, maxDelayMs = 5000, onRetry = () => {} } = {}
) {
  let attempt = 0;
...
function logger(level = process.env.LOG_LEVEL || 'info') {
  ...
  function isEnabled(l) {
    return levels.indexOf(l) >= idx;
  }
  return {
