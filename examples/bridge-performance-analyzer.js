...
class BridgePerformanceAnalyzer extends EventEmitter {
  ...
  startCPUMonitoring() {
    const timer = setInterval(() => {
      ...
      const cpuPercent =
        ((cpuUsage.user + cpuUsage.system) / 1000 / this.options.sampleInterval) * 100;
  ...
  generatePerformanceReport() {
    ...
    const memSamples = this.metrics.samples
      .filter((s) => s.type === 'memory')
      .map((s) => s.data.heapUsed);
    const cpuSamples = this.metrics.samples
      .filter((s) => s.type === 'cpu')
      .map((s) => s.data.cpuPercent);
    ...
    const report = {
      ...
      memory: {
        ...
        current:
          memSamples.length > 0
            ? `${Math.round(memSamples[memSamples.length - 1] / 1024 / 1024)}MB`
            : 'N/A',
        average:
          memSamples.length > 0
            ? `${Math.round(memSamples.reduce((a, b) => a + b) / memSamples.length / 1024 / 1024)}MB`
            : 'N/A',
        peak:
          memSamples.length > 0 ? `${Math.round(Math.max(...memSamples) / 1024 / 1024)}MB` : 'N/A',
      },
      cpu: {
        ...
        average:
          cpuSamples.length > 0
            ? `${(cpuSamples.reduce((a, b) => a + b) / cpuSamples.length).toFixed(1)}%`
            : 'N/A',
      ...
      network: {
        ...
        successRate:
          networkRequests.length > 0
            ? `${((networkRequests.filter((r) => r.result === 'success').length / networkRequests.length) * 100).toFixed(1)}%`
            : 'N/A',
        averageResponseTime:
          networkRequests.length > 0
            ? `${(networkRequests.reduce((sum, r) => sum + r.duration, 0) / networkRequests.length).toFixed(2)}ms`
            : 'N/A',
      },
      issues: {
    ...
    console.log(
      `🧠 Memory - Current: ${report.memory.current}, Average: ${report.memory.average}, Peak: ${report.memory.peak}`
    );
    console.log(
      `⚡ CPU - Current: ${report.cpu.current}, Average: ${report.cpu.average}, Peak: ${report.cpu.peak}`
    );
    console.log(
      `🌐 Network - Requests: ${report.network.totalRequests}, Success: ${report.network.successRate}, Avg Response: ${report.network.averageResponseTime}`
    );
    console.log(
      `⚠️  Issues - Memory Leaks: ${report.issues.memoryLeaks}, Suggestions: ${report.issues.optimizationSuggestions}\n`
    );
