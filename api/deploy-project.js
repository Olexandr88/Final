...
setInterval(() => {
  ...
  for (const [id, deployment] of deployments) {
    if (deployment.endTime && now - new Date(deployment.endTime).getTime() > 3600000) {
      // 1 hour
...
router.get('/deploy-project/stats', (req, res) => {
  ...
  res.json({
    ...
    successRate:
      deploymentArray.length > 0
        ? ((completed.length / deploymentArray.length) * 100).toFixed(1) + '%'
        : 'N/A',
...
router.post('/deploy-project', async (req, res) => {
  ...
  Promise.race([deploymentPromise, timeoutPromise]).catch((err) => {
  ...
  res.json({
    ...
    lastUpdated:
      deployment.logs.length > 0
        ? deployment.logs[deployment.logs.length - 1].timestamp
...
async function deployWithNitric(workDir, deployment) {
  try {
    ...
    const nitricProcess = await execPromise(
      'timeout 30s nitric start --ci || echo "Nitric start completed"',
      {
        cwd: workDir,
