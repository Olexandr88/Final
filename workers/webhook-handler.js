...
async function handleIssuesEvent(payload, env) {
  const { action, issue, repository: _repository } = payload;
  ...
  if (action === 'opened' || action === 'reopened') {
    ...
    const isUrgent = labels.some(
      (l) =>
        l.includes('security') || l.includes('critical') || l.includes('bug')
    );
...
async function handleIssueCommentEvent(payload, env) {
  const { action: _action, issue, comment } = payload;
 ...
 */
async function handlePullRequestEvent(payload, _env) {
 ...
 * Handle push events
 */
async function handlePushEvent(payload, _env) {
...
async function removeLabel(issueNumber, label, env) {
  ...
  await fetch(
    `https://api.github.com/repos/${owner}/${repo}/issues/${issueNumber}/labels/${label}`,
    {
      method: 'DELETE',
      headers: {
        Authorization: `token ${env.GITHUB_TOKEN}`,
 ...
 */
async function performMaintenance(_env) {
