  ...
  if (typeof config.validate === 'function') {
    ...
    for (const invalidConfig of invalidConfigs) {
      ...
      assert.equal(
        result.valid,
        false,
        `Invalid config should fail: ${JSON.stringify(invalidConfig)}`
      );
...
test('ProxyConfig platform-specific optimizations', async (t) => {
  ...
  if (platformConfig.platform === 'win32') {
    assert.ok(
      platformConfig.configDir.includes('AppData') ||
  ...
  if (platformConfig.platform === 'darwin') {
    assert.ok(platformConfig.configDir.includes('Library'), 'macOS should use Library directory');
  }

  if (platformConfig.platform === 'linux') {
    assert.ok(
      platformConfig.configDir.includes('.obfuscation-proxy'),
...
test('ProxyConfig security enhancements', async (t) => {
  ...
  if (defaults.allowedOrigins) {
    ...
    assert.ok(
      !defaults.allowedOrigins.includes('*') || defaults.allowedOrigins.length === 0,
...
test('ProxyConfig memory and performance benchmarks', async (t) => {
  ...
  assert.ok(
    ...
    `Memory usage should be reasonable (${(memoryIncrease / 1024).toFixed(2)}KB)`
  );
