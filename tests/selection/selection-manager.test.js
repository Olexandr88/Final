...
(async () => {
  ...
  const res = await manager.handleTextSelection(
    'tab-1',
    'This is an amazing result with excellent performance!',
    { pageType: 'form' }
  );
  ...
  // Test caching
  const res2 = await manager.handleTextSelection(
    'tab-1',
    'This is an amazing result with excellent performance!',
    { pageType: 'form' }
  );
  ...
  const opt = await manager.optimizeCurrentTab('tab-1', {
