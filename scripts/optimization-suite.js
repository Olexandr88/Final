...
class OptimizationSuite {
  ...
  async runOptimizationSuite() {
    ...
    try {
      ...
      console.log(
        `✅ Successful: ${this.results.optimizations.filter((o) => o.status === 'completed').length}`
      );
      console.log(
        `❌ Failed: ${this.results.optimizations.filter((o) => o.status === 'failed').length}`
      );
    } catch (error) {
  ...
  async generateSuiteReport() {
    ...
    const summary = {
      ...
      completedOptimizations: this.results.optimizations.filter((o) => o.status === 'completed')
        .length,
      ...
      averageOptimizationTime:
        this.results.optimizations.length > 0
          ? this.results.optimizations.reduce((sum, o) => sum + (o.duration || 0), 0) /
            this.results.optimizations.length
          : 0,
      successRate:
        (this.results.optimizations.filter((o) => o.status === 'completed').length /
          this.optimizationTasks.length) *
        100,
    };
