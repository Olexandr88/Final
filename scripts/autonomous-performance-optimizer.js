...
class AutonomousOptimizer {
  ...
  async runOptimization() {
    ...
    this.log(
      `Initial memory usage: ${initialMemory.heapUsed}MB (${initialMemory.heapUtilization}% heap utilization)`,
      'info'
    );
    ...
    try {
      ...
      this.log(
        `❌ Failed optimizations: ${this.results.failed}`,
        this.results.failed > 0 ? 'warning' : 'info'
      );
