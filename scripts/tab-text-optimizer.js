...
class TabOptimizationEngine {
  ...
  analyzeCurrentTabs() {
    ...
    console.log(
      `📊 Analyzed ${analysis.totalTabs} tabs with ${analysis.optimizationOpportunities} optimization opportunities`
    );
    return analysis;
  }
  ...
  calculateOptimizationScore(tab) {
    ...
    const resourcePenalty =
      tab.resourceUsage === 'high' ? -20 : tab.resourceUsage === 'medium' ? -10 : 0;
  ...
  analyzeSelectedText(text = null) {
    ...
    console.log(
      `📝 Analyzed text: ${analysis.wordCount} words, readability: ${analysis.readabilityScore}`
    );
    return analysis;
  }
  ...
  generateReport() {
    ...
    const successRate = (
      ((this.metrics.operations - this.metrics.errors) / this.metrics.operations) *
      100
    ).toFixed(2);
