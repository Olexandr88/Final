...
function githubRequest(path, options = {}) {
  return new Promise((resolve, reject) => {
    const [_owner, _repo] = CONFIG.GITHUB_REPO.split('/');
  ...
  try {
    ...
    return issues.filter(
      (issue) =>
        ...
        !issue.labels.some(
          (l) => l.name === 'agent:completed' || l.name === 'agent:in-progress'
        )
    );
  } catch (error) {
...
function calculatePriority(issue) {
  ...
  const ageInDays =
    (Date.now() - new Date(issue.created_at)) / (1000 * 60 * 60 * 24);
...
async function selectNextTask() {
  ...
  console.log(
    `📊 Found ${prioritized.length} issues. Top priority: #${prioritized[0].number} (score: ${prioritized[0].priority})`
  );
...
async function addLabelToIssue(issueNumber, label) {
  try {
    ...
    console.error(
      `⚠️ Error adding label ${label} to issue #${issueNumber}:`,
      error.message
    );
  }
}
...
async function addCommentToIssue(issueNumber, comment) {
  try {
    ...
    console.error(
      `⚠️ Error adding comment to issue #${issueNumber}:`,
      error.message
    );
  }
}
...
async function processTask(issue) {
  ...
  await addCommentToIssue(
    issue.number,
    `🤖 Autonomous agent is working on this issue...\\n\\nStarted at: ${new Date().toISOString()}`
  );
  ...
  try {
    ...
    try {
      const [owner, repo] = CONFIG.GITHUB_REPO.split('/');
      await githubRequest(
        `/repos/${owner}/${repo}/issues/${issue.number}/labels/agent:in-progress`,
        {
          method: 'DELETE',
        }
      );
    ...
    await addCommentToIssue(
      issue.number,
      `⚠️ Autonomous agent encountered an error:\\n\`\`\`\\n${error.message}\\n\`\`\`\\n\\nThe agent will retry later.`
    );
...
async function runOrchestrationLoop() {
  ...
  if (state.errorCount >= CONFIG.MAX_RETRIES) {
    console.error(
      `🛑 Too many consecutive errors (${state.errorCount}). Stopping for safety.`
    );
    console.error(
      '💡 Create a GitHub issue with label "agent:resume" to restart the agent.'
    );
...
function startHealthCheckServer() {
  ...
  server.listen(CONFIG.HEALTH_CHECK_PORT, () => {
    console.log(
      `💚 Health check server running on port ${CONFIG.HEALTH_CHECK_PORT}`
    );
  });
}
...
async function main() {
  ...
  console.log(
    `🔧 Auto-solve: ${CONFIG.ENABLE_AUTO_SOLVE ? 'Enabled' : 'Disabled'}`
  );
  console.log('='.repeat(50));
