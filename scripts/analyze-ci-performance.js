...
class CIPerformanceAnalyzer {
  ...
  calculateEfficiency(successRate, avgDuration, avgQueueTime) {
    ...
    return (
      Math.round(
        (successScore * successWeight + durationScore * durationWeight + queueScore * queueWeight) *
          10
      ) / 10
    );
  }
  ...
  displayResults(results) {
    ...
    if (bottlenecks.length === 0) {
      ...
      bottlenecks.forEach((bottleneck, i) => {
        console.log(
          `   ${i + 1}. [${bottleneck.severity.toUpperCase()}] ${bottleneck.description}`
        );
...
async function runPerformanceAnalysis() {
  try {
    ...
    const highPriorityOpts = results.optimizations.filter(
