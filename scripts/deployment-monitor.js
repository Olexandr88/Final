...
class DeploymentMonitor {
  ...
  async performHealthCheck(environment = 'production') {
    ...
    const baseUrl = environment === 'production' ? `https://${this.domain}` : this.devUrl;
  ...
  generateReport(environment) {
    ...
    const avgResponseTime =
      this.results.filter((r) => r.success).reduce((sum, r) => sum + r.duration, 0) / successful;
...
if (require.main === module) {
  ...
  switch (command) {
    ...
    default:
      console.log(
        'Usage: node deployment-monitor.js [check|monitor] [production|development] [interval_minutes]'
      );
      process.exit(1);
  }
}
