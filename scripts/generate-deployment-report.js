...
function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i++) {
    ...
    if (a === '--out') args.out = argv[++i];
      ...
      args.fromCi = ['1', 'true', 'yes'].includes(String(argv[++i]).toLowerCase());
      ...
      args.junit = ['1', 'true', 'yes'].includes(String(argv[++i]).toLowerCase());
...
async function httpGetJson(url, timeoutMs) {
  ...
  try {
    ...
    // Fallback to curl
    const { ok, out, error } = tryExec(
      `curl -fsSL --max-time ${Math.ceil(timeoutMs / 1000)} ${JSON.stringify(url)}`
    );
...
async function collectA2AStatus({ serverUrl, timeoutMs }) {
  const endpoints = ['/health', '/status', '/metrics', '/agents', '/connectivity'];
  const results = {};
...
function deriveSummary(report) {
  ...
  if (system.loadavg['1m'] > Math.max(1, system.cpu.count - 0.5)) {
    issues.push({
      level: 'warn',
      area: 'system',
      message: `High CPU load: ${system.loadavg['1m'].toFixed(2)}`,
    });
  }
  ...
  if (memFreePct < 0.1) {
    issues.push({
      level: 'warn',
      area: 'system',
      message: `Low free memory: ${(memFreePct * 100).toFixed(1)}%`,
    });
  }
  ...
  if (a2a['/health'] && a2a['/health'].status && a2a['/health'].status !== 'ok') {
    issues.push({
      level: 'error',
      area: 'a2a',
      message: `Health endpoint not ok: ${a2a['/health'].status}`,
    });
  }
  ...
  if (a2a['/connectivity'] && a2a['/connectivity'].agents) {
    ...
    if (down.length) {
      issues.push({
        level: 'error',
        area: 'agents',
        message: `${down.length} agents offline: ${down.map((d) => d.name).join(', ')}`,
      });
    }
  }
...
function toMarkdown(report) {
  ...
  md.push(
    `- Loadavg: 1m=${system.loadavg['1m'].toFixed(2)} 5m=${system.loadavg['5m'].toFixed(2)} 15m=${system.loadavg['15m'].toFixed(2)}`
  );
  md.push(
    `- Memory: rss=${system.memory.rss} heapUsed=${system.memory.heapUsed} free=${system.memory.free}/${system.memory.total}`
  );
  md.push(``);
  ...
  if (summary.warnings.length)
    md.push(`- Warnings: ${summary.warnings.map((w) => w.message).join('; ')}`);
  if (summary.errors.length)
    md.push(`- Errors: ${summary.errors.map((e) => e.message).join('; ')}`);
  md.push('');
  return md.join('\n');
}
...
function toJUnit(report) {
  ...
  if (health && !health.error) {
    ...
    testcases.push({
      name: 'a2a-health',
      ok: false,
      message: health ? health.message : 'no response',
    });
  }
  ...
  if (conn && !conn.error) {
    ...
    testcases.push({
      ...
      ok: down.length === 0,
      message: down.length ? `${down.length} offline` : '',
    });
  } else {
    testcases.push({
      name: 'a2a-connectivity',
      ok: false,
      message: conn ? conn.message : 'no response',
    });
  }
