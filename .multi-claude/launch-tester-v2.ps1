# Tester Claude Launcher V2
Set-Location "C:\Users\scarm"
$host.ui.RawUI.WindowTitle = "Tester Claude - Task Monitor"
...
$statusPath = ".multi-claude/status-tester.json"
$taskPath = ".multi-claude/tasks/tester-current-task.md"
$rolePath = ".multi-claude/claude-tester-task.md"
$sharedPath = ".multi-claude/shared"

# Initialize status
$initStatus = @{}
$initStatus.instance = "tester"
...
while ($true)
{
    if (Test-Path $taskPath)
    {
        ...
        if (($null -eq $lastProcessed) -or ($currentModified -gt $lastProcessed))
        {
            ...
            # Build prompt file
            $promptFile = ".multi-claude/prompt-tester.txt"

            "You are Tester Claude in a multi-Claude coordination system.`n`n" | Out-File $promptFile -Encoding utf8
