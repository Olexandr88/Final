...
while ($true)
{
    if (Test-Path $taskPath)
    {
        ...
        if (($null -eq $lastProcessed) -or ($currentModified -gt $lastProcessed))
        {
            ...
            # Build prompt file
            $promptFile = ".multi-claude/prompt-architect.txt"

            "You are Architect Claude in a multi-Claude coordination system.`n`n" | Out-File $promptFile -Encoding utf8
